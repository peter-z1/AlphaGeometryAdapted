...
class Problem:
    ...
    def txt(self) -> str:
        return (
            '; '.join([c.txt() for c in self.clauses]) +
            ' ? ' + self.goal.txt()
            if self.goal
            else ''
        )
        ...
        for clause in self.clauses:
            ...
            for c in clause.constructions:
                ...
                if len(c.args) != len(cdef.construction.args):
                    assert len(c.args) + \
                        len(clause.points) == len(cdef.construction.args)
    ...
    def from_txt(cls, data: str) -> Definition:
        ...
        for bs in basics:
            if ':' in bs:
                ...
                points = []
            if bs.strip():
                bs = [Construction.from_txt(b.strip())
...
class Theorem:
    ...
    def txt(self) -> str:
        ...
        conclusion_txt = ', '.join([clause.txt()
                                   for clause in self.conclusion])
        return f'{premise_txt} => {conclusion_txt}'

    def conclusion_name_args(
        self, mapping: dict[str, gm.Point]
    ) -> tuple[str, list[gm.Point]]:
        mapping = {arg: p for arg,
                   p in mapping.items() if isinstance(arg, str)}
        c = self.conclusion[0]
...
def why_eqratio(
    ...
    if why0:
        dep = Dependency(
            'eqratio', [a_, b_, c_, d_, e_, f_, g_, h_], '', level)
        dep.why = why0
        deps.append(dep)
...
class Dependency(Construction):
    ...
    def why_me(self, g: Any, level: int) -> None:
        ...
        if self.name == 'para':
            ...
            dep = Dependency('coll', [m, a, b], None,
                             None).why_me_or_cache(g, None)
            ...
            if whyeqangle:
                ...
                if diff:
                    d = Dependency(
                        'eqangle', [a, b, c, d, m, n, p, q], None, level)
                    d.why = whyeqangle
                    whyeqangle = [d]
                self.why += whyeqangle
